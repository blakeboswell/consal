...
- **`dco`** (existing, stays roughly as-is). Its job shrinks to exactly what
  ...
  from 638 to 348 lines, its README from 274 to ~90. `dco` today is
  exactly the container-lifecycle tool described here, nothing more.
...
- **Consal/`dco` interface (2026-07-14): plain `dco` + `devcontainer exec`,
  ...
  - **Sub-config contents:** a directory, matching how `--sub-config`
    ...
    **Correction (2026-07-14):** the same applies to Claude's own
    ...
    billing). This draws against an existing monthly plan instead of
  ...
  - **Container reuse, not fresh-per-task:** one container persists for
    the whole autonomous run, rebuilt only when the profile changes. Keeps
    `dco`'s existing persistence (`~/.claude` volume, bash history)
