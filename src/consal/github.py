...
matching `dco`'s own pattern of shelling out to external tools, and keeps
...
def close_issue(repo: str, issue_number: int, reason: str = "not planned") -> None:
    ...
    "not planned" (its documented default meaning for the latter, since
    this is cleanup of a disposable test issue, not real completed work).
    """
    subprocess.run(
