...
from eigen.scheduler import TickResult, next_turn, run_loop_once
...
@patch("eigen.scheduler.github.list_open_issues")
...
@patch("eigen.scheduler.state.state_dir", return_value=FAKE_STATE_DIR)
@patch("eigen.scheduler.container.ensure_container_up")
@patch("eigen.scheduler.state.read_active_issue")
@patch("eigen.scheduler.state.write_active_issue")
@patch("eigen.scheduler.github.list_open_issues")
def test_run_loop_once_returns_idle_tick_when_no_issues(
    ...
    result = run_loop_once("proj", Path("/workspace"), "owner/repo", "eigen")

    assert result == TickResult(issue_number=None, turn=None)
    assert result.was_idle is True
    mock_ensure_up.assert_called_once_with(Path("/workspace"), "eigen")


@patch("eigen.scheduler.state.state_dir", return_value=FAKE_STATE_DIR)
@patch("eigen.scheduler.container.ensure_container_up")
@patch("eigen.scheduler.container.run_turn")
@patch("eigen.scheduler.state.read_active_issue")
@patch("eigen.scheduler.state.write_active_issue")
@patch("eigen.scheduler.github.list_open_issues")
@patch("eigen.scheduler.github.comment_on_issue")
...
def test_run_loop_once_clears_active_issue_state_when_idle(
    ...
    run_loop_once("proj", Path("/workspace"), "owner/repo", "eigen")
